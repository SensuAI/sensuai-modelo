...
cap = cv2.VideoCapture("./testing/PlatesDataSet.mp4")
...
while ret:
    ...
    if ret and (frame_num % 30 == 0): 
        ...
        detections = coco_model(frame,verbose=True)[0]
        detections_ = []
        vehicles = []
        ...
        if datetime.now().strftime('%H:%M') == "02:00":
            write_csv(results, "./rawdata/raw_data_{}.csv".format(datetime.now().strftime("%Y-%m-%d")))
            results = {}
